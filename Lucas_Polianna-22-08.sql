...
-- alter table tbPedido modify ValorTotal decimal(10,2);
...
begin
	if not exists(select * from tbPedido where NotaFiscalPedido = vNotaFiscal) then
    ...
    insert into tbPedidoProduto(NotaFiscalPedido,CodigoBarras,Qtd,ValorItem) values (vNotaFiscal,vCodigoBarras,vQtd,vValorItem);
end
$$
...
/*
delimiter $$
create procedure spInsertCompra(vNotaFiscal int,vFornecedor decimal(14,0), vDataCompra date, vCodigoBarras decimal(14,0), vValorItem decimal(5,2), vQtd int)
begin
	if exists(select * from tbPedido where NotaFiscalPedido = vNotaFiscal) then
		set @QtdTotal = (select sum(Qtd) from tbPedidoProduto where NotaFiscalPedido = vNotaFiscal);
        set @ValorTot = (select sum(ValorItem) from tbPedidoProduto where NotaFiscalPedido = vNotaFiscal);
    else
		set @QtdTotal  = vQtd;
        set @ValorTot = vValorItem * vQtd;
        set @Fornecedor = (select IdFornecedor from tbFornecedor where CNPJ = vFornecedor);
		insert into tbPedido(NotaFiscalPedido,DataCompra,ValorTotal,QtdTotal,IdFornecedor) values (vNotaFiscal,vDataCompra,@ValorTot,@QtdTotal,@Fornecedor);
    end if;    
    insert into tbPedidoProduto(NotaFiscalPedido,CodigoBarras,Qtd,ValorItem) values (vNotaFiscal,vCodigoBarras,vQtd,vValorItem);
    update tbPedido set QtdTotal = @QtdTotal where NotaFiscalPedido = vNotaFiscal;
	update tbPedido set ValorTotal = @ValorTot where NotaFiscalPedido = vNotaFiscal;
end
$$
*/

delimiter $$
create procedure spInsertVenda(vcodigoCompra int,vCliente varchar(100), vDataCompra date, vCodigoBarras decimal(14,0), vValorItem decimal(5,2), vQtd int,vTotalVenda decimal (10,2))
begin
	if not exists(select * from tbProduto where CodigoBarras = vCodigoBarras) then
    select 'Este produto não existe';
	
    
    elseif not exists(select * from tbCliente where NomeCliente = vCliente) then
    select 'Este cliente não existe';
   
    else 
    insert into tbCompra(CodigoCompra,DataCompra,ValorTotal,QtdTotal, NotaFiscal,IdCli) values (vcodigoCompra,vCliente,vDataCompra,vCodigoBarras,vValorItem,vQtd,vTotalVenda);
     end if;  
end
$$
